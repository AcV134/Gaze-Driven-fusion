...
class LitModule(L.LightningModule):
    ...
    def __init__(self, *, model, optimizer, scheduler, criterion=None, evaluator=None, seg_pretrain=None, **kwargs):
        super().__init__()
        ...
        self.seg_pretrain_path = seg_pretrain 
    ...
    def _step(self, batch, evaluator=None):
        ...
        if evaluator:
            pred = {key:value[-1] for key, value in pred.items()}
    ...
    def on_fit_start(self):
        if self.seg_pretrain_path != None:
            pretrained_weights = torch.load(self.seg_pretrain_path)
            self.load_state_dict(pretrained_weights['state_dict'], strict=False)
            
            # # Freeze
            # for name, param in self.model.named_parameters():
            #     if 'ins_decoder' not in name:
    ...
    def configure_optimizers(self):
        ...
        # NOTE : For ReduceLROnPlateau scheduler
        scheduler = {'scheduler': scheduler, 'monitor': 'val/mIoU', 'interval': 'epoch', 'frequency': 1}
